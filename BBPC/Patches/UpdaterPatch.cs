...
namespace BBPC.Patches
{
    ...
    internal class UpdaterPatch
    {
        ...
        private static readonly string UpdateLocalizationKey = "BBPC_Menu_UpdateAvailable";
        private static readonly string ReminderLocalizationKey = "BBPC_Menu_Reminder";
        ...
        private static Transform GetDummyReminder(Transform parent)
        {
            if (dummyReminder == null)
            {
                ...
                // API.Logger.Info("已为MTM101BaldAPI创建虚拟对象DummyReminder");
            }
                    ...
                    if (stackFrames != null)
                    {
                        foreach (var frame in stackFrames)
                        {
                            ...
                            if (method != null && method.DeclaringType != null)
                            {
                                ...
                                {
                                    // API.Logger.Info($"拦截到来自MTM101BaldAPI对Reminder的访问 (方法: {method.DeclaringType.FullName}.{method.Name})");
                        ...
                        foreach (var frame in stackFrames)
                        {
                            ...
                            if (method != null && method.DeclaringType != null)
                            {
                                ...
                                if (namespaceName.StartsWith("MTM101BaldAPI") ||
                                    ...
                                    method.DeclaringType.FullName != null && method.DeclaringType.FullName.Contains("MTM101BaldAPI"))
                                {
                                    // API.Logger.Info($"拦截到通过GetChild从MTM101BaldAPI访问Reminder (方法: {method.DeclaringType.FullName}.{method.Name})");
